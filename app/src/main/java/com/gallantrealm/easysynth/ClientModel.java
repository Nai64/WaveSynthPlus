...
public class ClientModel {
	...
	private boolean extendedSliderRange;
	private int recordingSampleRate; // 0=native, 1=22050, 2=44100, 3=48000, 4=96000
	private int recordingBitDepth; // 0=16bit, 1=24bit, 2=32bit float
	...
	public void loadPreferences(Activity context) {
		...
		extendedSliderRange = preferences.getBoolean("extendedSliderRange", false);
		recordingSampleRate = preferences.getInt("recordingSampleRate", 0);
		recordingBitDepth = preferences.getInt("recordingBitDepth", 0);
	}
	...
	editor.putBoolean("extendedSliderRange", extendedSliderRange);
	editor.putInt("recordingSampleRate", recordingSampleRate);
	editor.putInt("recordingBitDepth", recordingBitDepth);
	editor.putBoolean("showCpuUsage", showCpuUsage);
	editor.commit();
}
	...
	public void setExtendedSliderRange(boolean extended) {
		extendedSliderRange = extended;
	}

	public boolean isExtendedSliderRange() {
		return extendedSliderRange;
	}

	public void setRecordingSampleRate(int rate) {
	...
	public void setRecordingBitDepth(int depth) {
		recordingBitDepth = depth;
	}

	public int getRecordingBitDepth() {
		return recordingBitDepth;
	}
		...
		if (fileName.startsWith("file:")) { // via an external url
			...
			if (object == null) {
				try {
					...
					// Use JSON deserializer for .wavesynth files, binary for .easysynth
					if (fileName.endsWith(".wavesynth")) {
						object = jsonDeserializeObject(is);
					} else {
	...
	private void jsonSerializeObject(Object object, OutputStream os) throws Exception {
		...
		try {
			if (object instanceof Sound) {
				// Use Sound's custom toJSON method for .wavesynth format
				String json = ((Sound) object).toJSON();
				w.write(json);
			} else {
				// Fallback to Gson for other objects
	...
	private Object jsonDeserializeObject(InputStream is) throws Exception {
		...
		Object object = null;
		try {
			// Try using Sound's custom fromJSON method first (supports .wavesynth format)
			object = Sound.fromJSON(json);
		} catch (Exception e1) {
			// Fallback to Gson if custom parsing fails
			try {
